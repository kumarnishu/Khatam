import { LeadUpdatableField } from "../models/crm/lead.fields.model"
import { ReferredParty } from "../models/crm/referred.model"
import { ExportLeadMobiles, ExportLeads } from "../utils/CrmUtils"
import { ILead, ILeadTemplate, IReferredParty, IRemark, TLeadBody, TLeadUpdatableFieldBody, TReferredPartyBody } from "../types/crm.types"
...
import { Remark } from "../models/crm/remark.model"
...
export const GetUselessLeads = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Number.isNaN(limit) && !Number.isNaN(page)) {
        ...
        let count = 0
        if (req.user?.is_admin) {
            leads = await Lead.find({ company: req.user?.company, stage: 'useless' }).populate('lead_owners').populate('updated_by').populate('company').populate('created_by').populate({
                path: 'remarks',
                populate: [
                    {
                        path: 'created_by',
            ...
            count = await Lead.find({ company: req.user?.company, stage: 'useless' }).countDocuments()
        }

        if (!req.user?.is_admin) {
            leads = await Lead.find({ company: req.user?.company, stage: 'useless', lead_owners: { $in: [req.user?._id] } }).populate('lead_owners').populate('updated_by').populate('company').populate('created_by').populate({
                path: 'remarks',
                populate: [
                    {
                        path: 'created_by',
            ...
            count = await Lead.find({ company: req.user?.company, stage: 'useless', lead_owners: { $in: [req.user?._id] } }).countDocuments()
        }
...
export const GetCustomers = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Number.isNaN(limit) && !Number.isNaN(page)) {
        ...
        let count = 0
        if (req.user?.is_admin) {
            leads = await Lead.find({ company: req.user?.company, is_customer: true }).populate('lead_owners').populate('updated_by').populate('company').populate('created_by').populate({
                path: 'remarks',
                populate: [
                    {
                        path: 'created_by',
            ...
            count = await Lead.find({ company: req.user?.company, is_customer: true }).countDocuments()
        }

        if (!req.user?.is_admin) {
            leads = await Lead.find({ company: req.user?.company, is_customer: true, lead_owners: { $in: [req.user?._id] } }).populate('lead_owners').populate('updated_by').populate('company').populate('created_by').populate({
                path: 'remarks',
                populate: [
                    {
                        path: 'created_by',
            ...
            count = await Lead.find({ company: req.user?.company, is_customer: true, lead_owners: { $in: [req.user?._id] } }).countDocuments()
        }
...
export const GetReminderRemarks = async (req: Request, res: Response, next: NextFunction) => {
    ...
    let reminders = await Remark.find({ remind_date: { $lte: new Date(), $gt: previous_date } }).populate('created_by').populate('updated_by').populate('company').populate({
        path: 'lead',
        populate: [
            {
                path: 'lead_owners',
...
export const GetRemarks = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (start_date && end_date) {
        ...
        remarks = await Remark.find({ created_at: { $gte: dt1, $lte: dt2 } }).populate('created_by').populate('updated_by').populate('company').populate({
            path: 'lead',
            populate: [
                {
                    path: 'lead_owners',
    ...
    if (!start_date && !end_date)
        remarks = await Remark.find({ created_at: { $gte: previous_date } }).populate('created_by').populate('updated_by').populate('company').populate({
            path: 'lead',
            populate: [
                {
                    path: 'lead_owners',
...
export const FuzzySearchLeads = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!Number.isNaN(limit) && !Number.isNaN(page)) {
        if (key.length == 1 || key.length > 4) {
            ...
            ).populate('lead_owners').populate('updated_by').populate('company').populate('created_by').populate({
                path: 'remarks',
                populate: [
                    {
                        path: 'created_by',
...
export const BackUpAllLeads = async (req: Request, res: Response, next: NextFunction) => {
    ...
    ExportLeads([])
    let leads = await Lead.find({ company: req.user?.company }).populate('created_by').populate('updated_by').populate('company').populate('lead_owners')
...
export const DeleteLead = async (req: Request, res: Response, next: NextFunction) => {
    const id = req.params.id;
    if (!isMongoId(id))
        return res.status(403).json({ message: "lead id not valid" })
    let lead = await Lead.findById(id);
    if (!lead) {
        return res.status(404).json({ message: "lead not found" })
    }
...
export const NewRemark = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.user)
        new_remark.company = req.user.company
    await new_remark.save()
...
export const BulkLeadUpdateFromExcel = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.file) {
        ...
        if (workbook_response.length > 4000)
            return res.status(400).json({ message: "should not be more than 4000 records" })
        ...
        for (let i = 0; i < workbook_response.length; i++) {
            ...
            if (validated) {
                if (!lead._id || !isMongoId(String(lead._id))) {
                    ...
                    if (req.user)
                        newlead.company = req.user.company
                    await newlead.save()
...
export const DeleteRemark = async (req: Request, res: Response, next: NextFunction) => {
    ...
    await Remark.findByIdAndDelete(rremark._id)
